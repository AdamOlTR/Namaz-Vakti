...
# Überprüfen, ob Chromium installiert ist
if command -v chromium-browser &> /dev/null
then
    echo "Chromium gefunden. Deinstalliere Chromium..."
    # Deinstalliere Chromium
    sudo apt-get remove --purge -y chromium-browser
    
    # Entferne nicht mehr benötigte Abhängigkeiten
    sudo apt-get autoremove -y
    
    # Lösche übrig gebliebene Konfigurationsdateien
    sudo apt-get purge -y chromium-browser
    
    # Lösche Chromium-Cache und Konfigurationsdateien im Home-Verzeichnis
    rm -rf ~/.config/chromium
    rm -rf ~/.cache/chromium
    
    echo "Chromium wurde erfolgreich deinstalliert."
else
    echo "Chromium ist nicht installiert."
fi
