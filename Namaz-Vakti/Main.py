...
try:
    ...
    def get_next_prayer_time(todays_times):
        now = datetime.now(pytz.timezone('Europe/Berlin'))  # Ensure the timezone is correct
        prayer_times = []

        for time_str in todays_times.iloc[0, 1:]:  # Skip the first column with the date
            prayer_time = datetime.strptime(time_str, '%H:%M').replace(year=now.year, month=now.month, day=now.day, tzinfo=pytz.timezone('Europe/Berlin'))
            if prayer_time < now:
    ...
    def update_namaz_times():
        ...
        if df is not None:
            ...
            if not todays_times.empty:
                ...
                html_file_path = os.path.join(os.path.expanduser("~"), "Namaz-Vakti", "Namaz-Vakti", "namaz_times.html")
    ...
    def open_chrome():
        file_path = os.path.abspath(os.path.join(os.path.expanduser("~"), "Namaz-Vakti", "Namaz-Vakti", "namaz_times.html"))
